from spider import parse_args, DEFAULT_DIR, ArgumentError, MissingArgumentError, MissingUrlError, BadArgumentError
...
for i, case in enumerate(positive_tests, 1):
	try:
		...
		assert actual == case["expected_result"], f"Expected {case['expected_result']}, got {actual}"
		print(f"Test {i} - {case['name']}: {GREEN_BOLD}OK!{RESET}")
	except AssertionError as e:
		print(f"Test {i} - {case['name']}: {BOLD}{RED}NOK!{RESET} {type(e).__name__}: {e}")
		failed_tests_count += 1
	except Exception as e:
		print(f"Test {i} - {case['name']}: {BOLD}{RED}NOK!{RESET} Unexpected exception: {e}")
		failed_tests_count += 1
...
for i, test in enumerate(negative_tests, 1):
	try:
		...
		print(f"Test {i} {test['name']}: {BOLD}{RED}NOK!{RESET} Unexpected exception: {type(e).__name__} (`{e}`); Expected: {test['expected_exception'].__name__}")
		failed_tests_count += 1
#######################################################################################
